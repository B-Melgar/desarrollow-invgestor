...
class transacciondetalleventaController extends Controller
{
    ...
    public function edit(Request $request){
        ...
        ->select('detalletransaccion.iddetalletransaccion', 'detalletransaccion.idtransaccion', 'detalletransaccion.idproducto',
                ...
                'detalletransaccion.subtotal', DB::raw('DATE_FORMAT(transaccion.fecha, "%d/%m/%Y") AS fecha'), 
                'proveedor.nombreproveedor','transaccion.idestadotransaccion', 'cliente.nombrecliente')
        ...
        return view ('inventario.transaccionventa.listadetalletransaccionventa', ["transacciondetalle"=>$transacciondetalle, "searchText" =>$idtransaccion])->with($data);
    }
    ...
    public function detalletransacciondataventa($idtransaccion){
        //dd($idtransaccion);
    ...
    public function detalletransacciondataventaedit($idtransaccion){
        //dd($idtransaccion);
        if (isset($idtransaccion)){  
            $transacciondetalle = null;
            $transacciondetalle = DB::table('detalletransaccion')
    ...
    public function agregardetalleventa(Request $request){
        ...
        $detalletransaccionmodelo->idproducto=$request->get('idproducto');
        $detalletransaccionmodelo->cantidad=$request->get('cantidad');
        $idprodu=$request->get('idproducto');
        ...
        ->where('producto.idproducto','=', $idprodu)->get();
        $precioventa = 0;
        $subtotal = 0;
        foreach ($producto as $prodd) {
            $precioventa =  $prodd->precioventa;
        }
        $cantidad = $request->get('cantidad');
        $subtotal = $precioventa * $cantidad;
        $detalletransaccionmodelo->precio=$precioventa;
        $detalletransaccionmodelo->subtotal=$subtotal;
        $detalletransaccionmodelo->save(); //Guardar o almacenar
        return Redirect::to('inventario/detalletransaccion'); //retorna al usuario a listado de grados a URL	
